...
- **History on your terms:** 1/3/6/12-month imports (Gmail All Mail, or Inbox/Sent), pause/resume, body-only style samples and a per-analysis token budget.
...
**Settings → Model → Embedding model** configures the separate embedding base URL, model ID, API key and OpenAI-compatible `/embeddings` or Ollama `/api/embed` protocol. **Test Connection** uses the entered fields without saving them and sends only a fixed test sentence, never mail; the model provider may charge for this request. **Settings → Search** controls indexing scopes, budgets and reviewed batches. Smart search defaults off. Select accounts, folders, content fields and 1/3/6/12 months; global AI permissions further restrict the text before any model request. Save, then choose **Index Now…** in Model or Search to review the model, endpoint, scope, token estimate and short excerpts before confirming one batch. Cancel keeps the preview for more detailed review; **Preview Next Batch** remains available without an AI call. Only new/changed permitted text is indexed. Each batch respects the global message limit, a 4,000–64,000 estimated-token budget and 50 overlapping text chunks; oversized messages remain keyword-searchable. Estimates are conservative UTF-8 counts, not exact provider billing. New mail needs another reviewed batch; there is no background paid indexing or automatic retry of failed/interrupted batches.
...
Gmail refresh checks up to 50 recent messages in each of Inbox, Sent, Drafts, Starred and All Mail, deduplicating overlaps. In **Settings → Mail**, select **All Gmail mail** and start a 1/3/6/12-month history import to page through older mail, including archived messages and custom labels. Spam and Trash are excluded from these provider fetches. Existing imports keep their saved scope until restarted.
...
Mail settings and Activity show import progress, safe failure reasons and recovery actions. Temporary network or 429/5xx errors during a read-only history fetch retry at most three times after 30, 120 and 300 seconds, preserving the checkpoint. Authentication, malformed pages, invalid cursors and database errors require explicit recovery. Pause/Resume keeps the cursor; restarting replaces the import. Old unexplained failed imports require Resume and are not silently restarted.
...
Model settings, permissions, and general preferences are **global across connected accounts**. Mail, drafts, skills, and workflow records belong to an individual account. Folder permissions apply to locally cached messages. Choose the history scope separately in Mail settings; enabling an AI scope alone does not download a folder. Gmail All Mail history includes archived messages; Outlook and IMAP history remain Inbox/Sent.
...
The installation's parent folder must be writable; Morrow never requests administrator elevation. Otherwise use the release downloads. Failed replacement or immediate launch errors restore the previous app; a later crash after a successful launch still needs manual recovery. The previous app remains in the sibling hidden `.morrow-update-…/previous` folder. Keep it until the new version works, then remove that old backup if desired. Interrupted/power-loss installs may require restoring this backup manually. Signature verification does not replace Apple notarization or Windows signing.
...
- New connections offer **1 / 3 / 6 / 12 months** (default 3). Gmail defaults to All Mail (excluding Spam/Trash); Outlook and IMAP use separate Inbox and Sent selections. Historical pages contain at most 50 messages and continue while the app is open. Existing history jobs retain their saved scope; start an All Gmail import to expand it. Gmail refresh checks five bounded scopes, while other providers retain their chosen Inbox/Sent scopes; continuous provider delta synchronization is not implemented.
