...
- Rust fixtures cover Gmail/Graph and IMAP/SMTP over isolated TLS, OAuth, all AI catalog workflows, learning, imports, schedules, search/index cancellation and budgets, original retry fingerprints, duplicate-owner IDs, shutdown, panic/disk-full transaction rollback, abrupt process death with on-disk journal recovery, writer exclusion and verified backup restore. `npm run rust:test` passed: rustfmt, clippy with warnings denied, 70 Rust tests, locked release/debug builds and all seven Node/Rust contracts.
