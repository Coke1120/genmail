...
| Gmail and Outlook | GenMail advertises both [1]. Morrow implements browser sign-in buttons with PKCE/state/browser-bound callbacks, paged 1/3/6/12-month Inbox/Sent import (legacy connections keep latest-50 Inbox sync until configured), and explicit manual sending. Native refresh follows the mailbox selected by a completed OAuth callback. Callback URLs are advanced registration details, not login links. Builds can include a publisher Google Desktop client for sign-in without entering credentials; current source also includes a public Microsoft desktop client ID for mail/calendar button-only sign-in. Advanced custom registration remains available for both providers. Google Cloud test-user/verification restrictions still apply; no authenticated provider testing is claimed. |
